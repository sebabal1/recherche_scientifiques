...
Notre époque fait face à deux défis importants en terme d'avencer technologique: \\
1- les respects de la vie privée. L'une des plus importantes est celle de la privatisation de ces données sur le web. Avec la protection des données (RGPD) promulgué en 2018, les données privées font entièrement partie de l'utilisateur, ces données ne peuvent pas être utilisé sans son accord.\\ 
2- L'union fait la force, or, le traitement en silo des données de chaque entreprise frein énormement l'évolution des apprentissage de machine learning. en effet, en partageant les données entre different secteur, les algorithmes pourraient s'enrichir d'autre contexte afin de prendre de meilleur décision en fonction du besoin de chaque application.\\
3- « Sans connaissance de son audience, et sans hypothèses préalables solides, la Big data ne sert à rien. » Benjamin Mercier, Responsable Digital Analytics, met en avant l'importance de la colecte des données. La masse d'information nécéssaire pour obtenir des apprentissages de qualité demande beaucoup de rigeur et d'expérience dans le domaine ciblé. Par exemple, comme précise l'article BLABLABLA, le secteur de BLABLABLA ne dispose pas de BLABLABLA \\

C'est grâce au Federated Learning(FL) que l'essort de l'industrie a relevé ces défis. Le FL est un processus d'apprentissage automatique qui vise à exploiter les îlots de données tout en gardant la sécurité des données. Des clients sont coordonnées avec un ou plusieurs serveurs pour faciliter une décentralisation d'un appentrissage automatique tel une architecture étoile. 

Le FL est basé sur les deux concepts que nous avons vu précédements le Machine Learning et le Deep Learning.

Les différents algos de ML et le DL effectuent des calculs basé sur leurs données de leur application. L'objectif du FL est de pouvoir agréger des information de diversse application, ou de différent acteurs afin de générer un modèle plus aboutie. Cette centralisation des données se fait généralement à l'aide d'n serveur centralisé comme le presente de schéma XXX de l'arclie des téléphones fig 4 avec comme exmple la collecte de données de smartphone :-D

SCHEMA !!! 

La pseudonyisation est donc un élement crutial pour le FL. L'article XXX met en avant telle ou telle techno avec tel ou tel avantage (BON CHANCE !!!)
